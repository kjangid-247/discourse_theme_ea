...
`react-src/paloma/index.tsx` is the only source file allowed to import `@paloma/core-ui` or `@paloma/core-ui/styles.css`.
...
Expose the same typed components through `react-src/paloma/adapter.ts`:

```tsx
const { Button, ThemeProvider } = getPaloma();
```

Features must use the adapter instead of reading `window.EaPaloma` directly.
...
| `pnpm build:paloma` | Builds the shared Paloma runtime, scopes CSS, and enforces its size budget. |
...
The shared Paloma runtime has a hard 10 MB gzip budget. `pnpm build:paloma` fails when the generated asset exceeds it.
